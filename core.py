...
# import mysql  # trying
# import mysql.vendor  # trying
# trying
import mysql.opentelemetry
# for handling the SIGINT signal
import signal
import random
import sqlite3
# main library
...
"""config ayarlari gelecek"""
# token: str = "token"
password: str = "love"
botAdminId: str = "user ID"
prefix: str = "n!"
botName: str = "Nick Gur"
autoUpdate: bool = bool(["F"])
...
async def query(string: str, databaseCursor: sqlite3.Cursor, ctx=None):
    try:
        ...
        database_res = databaseCursor.fetchall()
        ...
        if e.args[0] == "You can only execute one statement at a time." and ctx != None:
            ...
            log.fatal("Details about the lil cutie pie:")
            ...
            exit()
    if database_res.__len__() == 0:
        return None
    elif database_res.__len__() == 1:
        if database_res[0].__len__() == 1:
            return database_res[0][0]
        else:
            return [element for temp in database_res for element in temp]
    elif database_res.__len__() > 1:
        if database_res[0].__len__() == 1:
            return [element for temp in database_res for element in temp]
        else:
...
def updater():
    if autoUpdate:
        ...
        try:
            version = requests.get("version.txt raw link")
            changelog = requests.get("changelog.txt raw link")
        ...
        if fetchedVersion > VERSION:
            ...
            if (input("Do you want to update? (y/n) ") == "y"):
                ...
                try:
                    ...
                    coreDownloaded = requests.get("update link will be here")
...
if os.path.exists(os.path.abspath("Data/users.sqlite3")):
    log.info("User database found.")
    try:
        userDB = sqlite3.connect(os.path.abspath("Data/users.sqlite3"))
    ...
    try:
        ...
        userDBCommand.execute(
            "CREATE TABLE Users(userid VARCHAR(18) PRIMARY KEY, username VARCHAR(32) NOT NULL, lastdrop TIME, lastgrab TIME)")

        log.info("User database created.")
        userDBCommand.execute(
            "CREATE TABLE OwnedCards (cardid INTEGER PRIMARY KEY, cardname VARCHAR(32) NOT NULL, series VARCHAR(32) NOT NULL, rarity SMALLINT NOT NULL check (rarity IN(1, 2, 3, 4, 5)), condition SMALLINT NOT NULL check (condition IN(1, 2, 3, 4, 5)), userid VARCHAR(18) REFERENCES Users(userid) ON UPDATE CASCADE ON DELETE SET NULL)")

        log.info("Owned cards database created.")
        userDBCommand.execute(
            "CREATE TABLE Wishlist (cardnumber INTEGER PRIMARY KEY, cardname VARCHAR(32) NOT NULL UNIQUE, series VARCHAR(32) NOT NULL, rarity SMALLINT NOT NULL check (rarity IN(1, 2, 3, 4, 5)), userid VARCHAR(18) REFERENCES Users(userid) ON UPDATE CASCADE ON DELETE SET NULL)")

        log.info("Wishlist database created.")
        userDBCommand.execute(
            "CREATE TABLE Inventory (itemid INTEGER PRIMARY KEY, itemname VARCHAR(32) NOT NULL, itemquantity SMALLINT NOT NULL, userid VARCHAR(18) REFERENCES Users(userid) ON UPDATE CASCADE ON DELETE SET NULL)")
...
if os.path.exists(os.path.abspath("Data/cards.sqlite3")):
    log.info("Card database found.")
    try:
        cardDB = sqlite3.connect(os.path.abspath("Data/cards.sqlite3"))
    ...
    try:
        ...
        cardDBCommand.execute(
            "CREATE TABLE Cards (cardnumber INTEGER PRIMARY KEY, cardname VARCHAR(32) NOT NULL UNIQUE, series VARCHAR(32) NOT NULL, rarity SMALLINT NOT NULL check (rarity IN (1, 2, 3, 4, 5)), filename VARCHAR(256) NOT NULL,  dropped BIGINT NOT NULL DEFAULT 0)")
...
if os.path.exists(os.path.abspath("Data/servers.sqlite3")):
    log.info("Server database found.")
    try:
        serverDB = sqlite3.connect(os.path.abspath("Data/servers.sqlite3"))
    ...
    try:
        ...
        serverDBCommand.execute(
            "CREATE TABLE Servers (serverid VARCHAR (18) PRIMARY KEY, servername VARCHAR(32) NOT NULL, channelid VARCHAR(18) NOT NULL, adminid VARCHAR(18))")
...
async def on_ready():
    ...
    log.info("==================================================")
    log.info("Properties of config.ini:")
    ...
    for guild in bot.guilds:
        if serverList == None:
            ...
            await query(f"INSERT INTO Servers VALUES ({guild.id}, '{guild.name}', {mainChannel.id}, {botAdminId})",
                        serverDBCommand)
        else:
            if str(guild.id) in serverList:
                mainChannel = guild.get_channel(
                    int(await query(f"SELECT channelid FROM Servers WHERE serverid = {guild.id}", serverDBCommand)))
                if mainChannel == None:
                    channel = await guild.create_text_channel(botName)
                    mainChannel = bot.get_channel(channel.id)
                    await query(
                        f"INSERT INTO Servers VALUES ({guild.id}, '{guild.name}', {mainChannel.id}, {botAdminId})",
                        serverDBCommand)
            else:
                channel = await guild.create_text_channel(botName)
                mainChannel = bot.get_channel(channel.id)
                await query(f"INSERT INTO Servers VALUES ({guild.id}, '{guild.name}', {mainChannel.id}, {botAdminId})",
                            serverDBCommand)
        await mainChannel.send("**" + botName + "(by aasamiya) is online!**")
        await mainChannel.send("Mommy Asamiya :3")
    log.info(
    ...
    if serverList == None:
        ...
        if str(guild.id) in serverList:
            ...
            await query(f"INSERT INTO Servers VALUES ({guild.id}, '{guild.name}', {mainChannel.id}, {botAdminId})",
                        serverDBCommand)
    await mainChannel.send("**" + botName + "(by aasamiya) is online!**")
    await mainChannel.send("thx mommy")
...
async def dropcard(ctx):
    ...
    if userList == None:
        await query(
            f"INSERT INTO Users (userid, username, lastdrop) VALUES ({ctx.author.id}, '{ctx.author.name}', '{dropTimeStr}')",
            userDBCommand, ctx)
    elif str(ctx.author.id) not in userList:
        await query(
            f"INSERT INTO Users (userid, username, lastdrop) VALUES ({ctx.author.id}, '{ctx.author.name}', '{dropTimeStr}')",
            userDBCommand, ctx)
    elif str(ctx.author.id) in userList and await query(f"SELECT lastdrop FROM Users WHERE userid = {ctx.author.id}",
                                                        userDBCommand, ctx) == None:
        ...
        lastDrop = datetime.strptime(
            await query(f"SELECT lastdrop FROM Users WHERE userid = {ctx.author.id}", userDBCommand, ctx), "%H:%M:%S")

    if (datetime.strptime(dropTimeStr, "%H:%M:%S") - lastDrop).seconds < 300 and str(ctx.author.id) != botAdminId:
        await ctx.send(ctx.author.mention + ", you can drop a card only every 5 minutes :3", delete_after=60)
        return
    else:
    ...
    cardNames = await query(
        f"SELECT cardname FROM Cards WHERE filename = '{cardsToDrop[0]}' OR filename = '{cardsToDrop[1]}' OR filename = '{cardsToDrop[2]}'",
        cardDBCommand)
    imageCreation(cardsToDrop, GREY, os.path.abspath("Temp/drop.png"))

    """message for wishlists"""
    whoWishlisted = await query(
        f"SELECT userid FROM Wishlist WHERE cardname = '{cardNames[0]}' OR cardname = '{cardNames[1]}' OR cardname = '{cardNames[2]}'",
        userDBCommand)
    ...
    dropEmbed = discord.Embed(title="Dropped Cards", description=ctx.author.mention + " dropped the following cards!",
    ...
    def check(reaction, user):
        if str(reaction.emoji) == "1️⃣" and user.id != bot.user.id and reaction.message.id == drop.id:
            return user
        elif str(reaction.emoji) == "2️⃣" and user.id != bot.user.id and reaction.message.id == drop.id:
            return user
        elif str(reaction.emoji) == "3️⃣" and user.id != bot.user.id and reaction.message.id == drop.id:
            return user
    ...
    for _ in range(3):
        ...
        if userList == None:
            await query(
                f"INSERT INTO Users (userid, username, lastgrab) VALUES ({user.id}, '{user.name}', '{lastGrabStr}')",
                userDBCommand, ctx)
        elif str(user.id) not in userList:
            await query(
                f"INSERT INTO Users (userid, username, lastgrab) VALUES ({user.id}, '{user.name}', '{lastGrabStr}')",
                userDBCommand, ctx)
        elif str(user.id) in userList and await query(f"SELECT lastgrab FROM Users WHERE userid = {user.id}",
                                                      userDBCommand) is None:
            ...
            lastGrab = datetime.strptime(
                await query(f"SELECT lastgrab FROM Users WHERE userid = {user.id}", userDBCommand), "%H:%M:%S")
        ...
        if (datetime.strptime(lastGrabStr, "%H:%M:%S") - lastGrab).seconds > 300 or str(user.id) == botAdminId:
            ...
            cardDropped = await query(f"SELECT cardname, series, rarity FROM Cards WHERE filename = '{cardPath}'",
                                      cardDBCommand)
            cardCondition = random.choice([1, 1, 2, 2, 2, 3, 3, 3, 3, 3, 3, 3, 4, 4, 5])
            await query(
                f"INSERT INTO OwnedCards (cardname, series, rarity, condition, userid) VALUES ('{cardDropped[0]}', '{cardDropped[1]}', {cardDropped[2]}, {cardCondition}, {user.id})",
                userDBCommand)
            ...
            if wishlist == cardDropped[0]:
                await query(f"DELETE FROM Wishlist WHERE userid = {user.id} AND cardname = '{cardDropped[0]}'",
                            userDBCommand)
                await ctx.send(user.mention + " has received their wishlisted card!", delete_after=60)
        else:
            await ctx.send(user.mention + ", you can grab a card only every 5 minutes :3", delete_after=60)
    await drop.delete()
...
async def cooldown(ctx):
    ...
    lastDrop, lastGrab = [datetime.strptime(element, "%H:%M:%S") for element in
                          await query(f"SELECT lastdrop, lastgrab FROM Users WHERE userid = {ctx.author.id}",
                                      userDBCommand)]

    """embed creation and sending"""
    cooldownEmbed = discord.Embed(title="Cooldown", description=ctx.author.mention + " your cooldown is:",
                                  color=discord.Color.dark_purple())
    if lastDrop is not None:
        dropCooldown = f"You have to wait {300 - (actualTime - lastDrop).seconds} seconds!" if (
                                                                                                       actualTime - lastDrop).seconds < 300 else "You can drop cards now!"
        cooldownEmbed.add_field(name="Drop", value=dropCooldown, inline=True)
    if lastGrab is not None:
        grabCooldown = f"You have to wait {300 - (actualTime - lastGrab).seconds} seconds!" if (
                                                                                                       actualTime - lastGrab).seconds < 300 else "You can grab a card now!"
        cooldownEmbed.add_field(name="Grab", value=grabCooldown, inline=True)
    if lastGrab is None or lastDrop is None:
...
async def collection(ctx, mention: discord.Member = None):
    ...
    userCollection = await query(
        f"SELECT cardid, cardname, series, rarity, condition FROM OwnedCards WHERE userid = {user.id}", userDBCommand)
    ...
    if userCollection == None:
        ...
        if type(userCollection[0]) == tuple:
            for card in userCollection:
                collectionEmbed.add_field(name=f"{card[1]} - {card[2]}",
                                          value=f"Rarity: '{parseRarity(card[3])}'\nCondition: '{parseCondition(card[4])}'\nID: {card[0]}",
                                          inline=True)
        else:
            collectionEmbed.add_field(name=f"{userCollection[1]} - {userCollection[2]}",
                                      value=f"Rarity: '{parseRarity(userCollection[3])}'\nCondition: '{parseCondition(userCollection[4])}'\nID: {userCollection[0]}",
                                      inline=True)
...
async def wishlist(ctx, mention: discord.Member = None):
    user = ctx.author if mention is None else mention
    ...
    if userWishlist == None:
        ...
        if type(userWishlist[0]) == tuple:
            for card in userWishlist:
                wishlistEmbed.add_field(name=f"{card[0]} - {card[1]}", value=f"Rarity: {parseRarity(card[2])}",
                                        inline=False)
        else:
            wishlistEmbed.add_field(name=f"{userWishlist[0]} - {userWishlist[1]}",
                                    value=f"Rarity: {parseRarity(userWishlist[2])}", inline=False)
...
async def wishlistadd(ctx, card):
    ...
    if cardLower in cardList:
        ...
        if currentWishlist == None:
            cardToAdd = await query(f"SELECT cardnumber, cardname, series, rarity FROM Cards WHERE cardname = '{card}'",
                                    cardDBCommand, ctx)
            await query(
                f"INSERT INTO Wishlist (cardnumber, cardname, series, rarity, userid) VALUES ({cardToAdd[0]}, '{cardToAdd[1]}', '{cardToAdd[2]}', {cardToAdd[3]}, {ctx.author.id})",
                userDBCommand)
        else:
            ...
            if cardLower in currentWishlist:
                ...
                cardToAdd = await query(
                    f"SELECT cardnumber, cardname, series, rarity FROM Cards WHERE cardname = '{card}'", cardDBCommand,
                    ctx)
                await query(
                    f"INSERT INTO Wishlist (cardnumber, cardname, series, rarity, userid) VALUES ({cardToAdd[0]}, '{cardToAdd[1]}', '{cardToAdd[2]}', {cardToAdd[3]}, {ctx.author.id})",
                    userDBCommand)
        ...
        await ctx.send(ctx.author.mention + ", that card doesn't exist! Check if you have spelled the name correctly.",
                       delete_after=60)
    ...
    if cardLower in cardList:
        ...
        if currentWishlist == None:
            ...
            if cardLower not in currentWishlist:
                ...
                await query(f"DELETE FROM Wishlist WHERE cardname = '{card}' AND userid = {ctx.author.id}",
                            userDBCommand, ctx)
...
async def burn(ctx, card):
    ...
    if card == "all":
        ...
        if collection == None:
            ...
            message = await ctx.send(ctx.author.mention + ", are you sure you want to burn all of your cards?",
                                     delete_after=60)
            allFlag = True
    else:
        """checks if card is in card list"""
        if cardLower in cardList:
            ...
            if collection == None:
                ...
                if cardLower not in collection:
                    ...
                    return
                message = await ctx.send(ctx.author.mention + ", are you sure you want to burn this card?",
                                         delete_after=60)
        else:
            await ctx.send(
                ctx.author.mention + ", that card doesn't exist! Check if you have spelled the name correctly.",
                delete_after=60)
            return
    ...
    def check(reaction, user):
        if str(reaction.emoji) == "✅" and user.id != bot.user.id and reaction.message.id == message.id:
            return user
        elif str(reaction.emoji) == "❌" and user.id != bot.user.id and reaction.message.id == message.id:
            return user
    ...
    if str(reaction.emoji) == "✅" and user.id == ctx.author.id:
        if allFlag:
            ...
            await query(f"DELETE FROM OwnedCards WHERE cardname = '{card}' AND userid = {ctx.author.id}", userDBCommand,
                        ctx)
